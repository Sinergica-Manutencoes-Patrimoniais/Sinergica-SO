...
|----|------|----------|------------|----------------|--------|
| 1 | Domínio: form de item (código/identificação obrigatório), remove `quantidadeTotal` como entrada; `quantidade` = contagem derivada de unidades | AC-1,AC-3 | — | vitest | todo |
| 2 | Migration aditiva (se necessário): `codigo` obrigatório/único no item; nada destrutivo | AC-1,AC-4 | 1 | lint:migrations | todo |
| 3 | Adapter: criar item = criar unidade; listar por item; contagem por tipo/categoria | AC-1,AC-2,AC-3 | 1 | typecheck | todo |
| 4 | UI `FerramentasPage`: cadastro 1-item, sem campo quantidade; lista item-a-item com agrupamento por categoria | AC-1,AC-3 | 3 | typecheck | todo |

...
sequenceDiagram
    ...
    alt NEW.updated_by = sentinela Auvo
    ...
    alt descriptor.writeEnabled = false
        ...
        AUVO-->>PUSH: auvo_id
        PUSH->>TBL: UPDATE auvo_id/auvo_sync_status='synced'/auvo_synced_at (updated_by=sentinela)
        PUSH->>OUT: marca 'sent'
    end
...
2. **`supabase/migrations/0024_E01-S22_auvo_sync_outbox.sql`** — tabela `pcm.auvo_sync_outbox` +
   função `pcm.fn_auvo_enqueue()` (trigger genérica, `TG_ARGV[0]` = chave da entidade) + reserva
   do sentinela `auvo_system_user_id` (ver `domain.md`).
...
- pgTAP: `force row level security` na outbox, sem policy para `authenticated` (nem SELECT),
  `fn_auvo_enqueue` não enfileira quando `updated_by` = sentinela.
...
| Sentinela anti-loop mal aplicado | Se um write inbound esquecer de gravar `updated_by=sentinela`, gera loop write→Auvo→webhook→write→... | baixa × alto | Testado explicitamente no pgTAP/integração desta story e reforçado por code review em toda story de entidade que usa o registry |

...
| **[RESOLVIDO em `E01-S22`, achado ao mapear catálogos das stories `E01-S25`/`E01-S27`/`E01-S28`]** Nem todo recurso Auvo tem campo `active`, endpoint `PATCH`, ou aceita `externalId` no `POST` | `Segments`/`Keywords`/`Categorias`/`Customer Groups` não têm `active` (só `DELETE` físico); `Customer Groups`/`Teams` não têm `PATCH` documentado; `Task Types`/`Segments`/`Keywords`/`Customer Groups`/`Teams` não aceitam `externalId` no `POST` (idempotência de criação mais fraca — mitigada por match-by-description antes de criar, ver spec.md de cada entidade); `Users` usa `unavailableForTasks:true` em vez de `active:false` para "desativar" | média × médio (por entidade, não sistêmico) | `AuvoEntityDescriptor` ganhou 3 campos opcionais aditivos: `deleteStrategy` (`'soft-patch'` padrão \| `'hard-delete'`), `deactivatePatch` (patch customizado quando o campo não é `active`), `supportsUpdate` (`false` pula `PATCH` inexistente, no-op de sucesso). `pcm-auvo-push` já implementa os 3; cada story de entidade só declara os valores que precisar |
| **[RESOLVIDO em `E01-S22`, achado ao mapear `E01-S31`]** Nome do campo de idempotência de criação varia — `Services` usa `externalCode`, não `externalId`; `id` de `Services` é GUID (string), não bigint | `pcm-auvo-push` hardcodava `externalId: row.row_id` no `POST` — quebraria a idempotência de Services silenciosamente (Auvo não reconheceria o campo, cada retry criaria um serviço novo) | alta × alto | Campo aditivo `externalIdField` no `AuvoEntityDescriptor` (padrão `'externalId'`), `pcm-auvo-push` usa `descriptor.externalIdField ?? 'externalId'`. **Pendente, não bloqueante**: a tipagem TS de `auvo_id`/`auvoId` no drain assume `number` genericamente — funciona em runtime (JS não é fortemente tipado), mas seria mais correto tipar como `number \| string`; ajustar quando `E01-S31` implementar de verdade |
| **[RESOLVIDO em `E01-S22`, achado ao mapear `E01-S32`]** `Teams` não tem `PATCH` nem `DELETE` documentado — só `POST`/`GET` | Sem essa opção no registry, uma exclusão de Equipe no PCM chamaria `auvoPatch`/`auvoDelete` contra um endpoint que não existe, gerando erro permanente no outbox para toda exclusão de equipe | média × médio | Terceiro valor em `deleteStrategy`: `'unsupported'` — exclusão fica só local (soft-delete no PCM), nenhuma chamada ao Auvo. A UI de `E01-S32` deve avisar isso ao usuário (mesmo espírito do aviso de `supportsUpdate:false` em `E01-S27`) |

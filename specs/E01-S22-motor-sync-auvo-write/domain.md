...
| **GUC de sync** (`app.auvo_sync_write`) | Parâmetro transacional (`set_config(..., true)`, local à transação) setado pela RPC `pcm.fn_apply_auvo_sync` antes de aplicar um patch que se origina de dados vindos do Auvo (drain confirmando sucesso, dispatcher de webhook/poller de `E01-S23`). Existe só para o anti-loop — `fn_auvo_enqueue` não enfileira quando esse GUC está `true` na transação corrente. Substitui a ideia inicial de um `updated_by` sentinela (descartada: essa coluna tem `references auth.users`, exigiria linha falsa na tabela gerenciada pelo Supabase Auth). | Feature flag de produto (`config.feature_flags`) — este é um parâmetro de sessão interno do motor, nunca lido pela UI |
| **`fn_apply_auvo_sync`** | RPC `security definer` que aplica um patch jsonb genérico numa linha de `pcm.<tabela>` (usada tanto pelo drain de `E01-S22` quanto pelo dispatcher/poller de `E01-S23`), setando o GUC de sync antes do `UPDATE` na mesma transação. É o único caminho de escrita que a Anti-Corruption Layer usa para gravar dados vindos do Auvo de volta no PCM. | Adapter de UI (`supabase-<entidade>-adapter.ts`) — grava dados vindos do usuário, nunca seta o GUC |

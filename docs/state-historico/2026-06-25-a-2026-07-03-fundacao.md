---
name: state-historico-2026-06-25-a-2026-07-03
description: Arquivo do STATE.md — período de fundação do projeto (2026-06-25 a 2026-07-03: E00-S05 a S10, E01-S09 a S11, decisões RBAC/Auvo iniciais). Formato antigo (pré-convenção de entradas por sessão). Não carregado por padrão.
alwaysApply: false
---

# STATE — arquivo histórico (fundação, 2026-06-25 a 2026-07-03)

> Movido de `docs/STATE.md` na rotação de 2026-07-13. Este trecho usava um formato mais antigo (seções fixas: Status geral, Decisões recentes, Bloqueios) em vez das entradas cronológicas por sessão que vieram depois. **Muitos itens de "Bloqueios" aqui já estão resolvidos** (marcados ~~riscado~~ no próprio texto) — não tratar como estado atual sem conferir. Ver `docs/state-historico/INDEX.md`.

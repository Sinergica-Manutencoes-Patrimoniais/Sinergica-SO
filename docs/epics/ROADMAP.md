...
| E01-S12 | Visão 360 do Cliente — página read-only por condomínio (v1: cadastro + backlog GUT + histórico de OS; +técnicos/equipamentos se E01-S11 fechar). PMOC/ativos/financeiro/WhatsApp adiados p/ fase 2 | [product](../../specs/E01-S12-visao-360-cliente/product.md) · [spec](../../specs/E01-S12-visao-360-cliente/spec.md) | **spec.md pronta** (aprovada por Lucas 2026-07-03, tier Pequeno — 3 decisões de negócio resolvidas no product.md §6; próximo: `@sm` → tasks.md) — 2 OPEN-QUESTION não-bloqueantes: (1) como vincular técnicos a cliente (schema não suporta → v1 cobre só equipamentos); (2) separar cancelada de concluída no histórico (refinamento fase 2) | Claude (sessão Lucas) | — |

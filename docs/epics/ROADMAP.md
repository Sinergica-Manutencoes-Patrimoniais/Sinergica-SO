...
| E01-S09 | Integração Auvo — Fundação: cliente HTTP, sync de clientes, criação de task ao entrar em `planejamento` | [spec](../../specs/E01-S09-integracao-auvo-fundacao/spec.md) · [design](../../specs/E01-S09-integracao-auvo-fundacao/design.md) · [tasks](../../specs/E01-S09-integracao-auvo-fundacao/tasks.md) | **PR [#10](https://github.com/Sinergica-Manutencoes-Patrimoniais/Sinergica-SO/pull/10) aberto** — gates Node verdes (`lint:migrations`/`audit-esteira`/`eval-spec-fidelity`); Edge Functions (Deno) não puderam ser type-checked/executadas nesta sessão (sem Deno CLI); 6 SPEC_DEVIATION abertos, ver tasks.md | Claude (sessão Lucas) | ⏳ |

...
| E00-S08 | Renomear papéis RBAC — admin/escritorio/tecnico → superadmin/supervisor/colaborador (mesma matriz de permissão) + provisiona primeiro superadmin | [spec](../../specs/E00-S08-renomear-papeis-rbac/spec.md) | Implementado — usuário `sinergicaengenharia@gmail.com` já provisionado (papel `admin`, migration `0004` remapeia para `superadmin` no merge) | Claude (sessão Lucas) | ⏳ |

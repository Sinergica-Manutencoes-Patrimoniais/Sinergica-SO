...
| E01-S11 | Integração Auvo — Sync de técnicos/equipes/equipamentos (Auvo → PCM, cache read-only) | [spec](../../specs/E01-S11-integracao-auvo-sync-tecnicos-equipamentos/spec.md) · [tasks](../../specs/E01-S11-integracao-auvo-sync-tecnicos-equipamentos/tasks.md) | **Implementado por `@dev` (branch `feat/E01-S11-integracao-auvo-sync-tecnicos-equipamentos`, ainda sem PR)** — migrations `0012` (cache `pcm.tecnicos_cache`/`pcm.equipamentos_cache` + RLS FORCE + `grant usage on schema pcm to service_role` que faltava) e `0013` (pg_cron diário 06:00 UTC, reusa secrets do Vault de `0011`); Edge Functions `pcm-auvo-users-sync`/`pcm-auvo-equipment-sync` + `_shared/auvo/paginate.ts` + pgTAP RLS. Gates Node verdes (lint:migrations/lint/typecheck/test/build); sem Deno CLI/Docker aqui p/ type-check+testes Deno e pgTAP (mesma ressalva de `E01-S09`/`E01-S10`); task 8 (habilitar `pg_cron` no Dashboard) é operacional pendente. Nenhum SPEC_DEVIATION; 1 [AUTO-DECISION] a confirmar com o lead (pular soft-delete em resultado vazio). **Revisão @qa (Quinn) 2026-07-03: CONCERNS — NÃO liberado p/ merge ainda.** Código-fonte e schema implementam corretamente AC-1..AC-5 (segurança do cache até mais estrita que o testado); mas o pgTAP `tecnicos_equipamentos_cache_rls.test.sql` vai **falhar no job `db-tests` da CI**: as tabelas de cache dão a `authenticated` só `GRANT SELECT`, então `UPDATE`/`DELETE` (linhas 51/57/65/70, fora de `throws_ok`) levantam `42501 permission denied` e abortam a transação em vez de "filtrar silenciosamente 0 linhas" como o teste assume (modelo copiado de `pcm.clientes`, que tem grant de update). Correção = envolver os UPDATE/DELETE em `throws_ok(..., '42501', ...)` (@dev) + confirmar `db-tests` verde na CI. **→ CORRIGIDO por @dev (Dex): os 4 blocos UPDATE/DELETE agora usam `throws_ok('42501')` (mesmo padrão dos INSERT); `lint:migrations` segue verde; falta só a CI confirmar `db-tests` (sem Docker aqui).** | Claude (sessão Lucas) | ⏳ (blocker pgTAP do @qa corrigido; AC verificáveis na CI/local Deno+Docker) |

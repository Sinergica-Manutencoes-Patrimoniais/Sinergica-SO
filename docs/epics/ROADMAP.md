...
| E00-S05 | Autenticação e Autorização — Supabase Auth real + RBAC (substitui bypass dev) | [spec](../../specs/E00-S05-autenticacao-autorizacao/spec.md) | Código implementado — aguardando validação com Docker/Supabase local + @qa (ver tasks.md) | Claude (sessão Lucas) | ⏳ (código pronto, gates de banco não executados) |

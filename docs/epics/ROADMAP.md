...
| E01-S22 | Motor de sync Auvo (write path) — outbox transacional `pcm.auvo_sync_outbox` + drain genérico `pcm-auvo-push` + scaffold do entity registry + `auvoPatch`/`auvoDelete`. Tier arquitetural, ADR-0005 | [product](../../specs/E01-S22-motor-sync-auvo-write/product.md) · [design](../../specs/E01-S22-motor-sync-auvo-write/design.md) · [domain](../../specs/E01-S22-motor-sync-auvo-write/domain.md) · [spec](../../specs/E01-S22-motor-sync-auvo-write/spec.md) · [tasks](../../specs/E01-S22-motor-sync-auvo-write/tasks.md) | **Implementado localmente** (branch `feat/E01-S22-motor-sync-auvo-write`, aguardando push/PR) — migrations `0024`/`0025`, `pcm-auvo-push`, registry scaffold, `auvoPatch`/`auvoDelete`. Gates Node verdes (lint/typecheck/test 126 pass/build/esteira/eval/arch); Deno (3 `*.test.ts`) e pgTAP (`auvo_sync_outbox_rls.test.sql`) não executados aqui — sem Deno CLI/Docker, confirmar no CI `db-tests` antes do merge. 2 divergências de design resolvidas durante a implementação (GUC em vez de sentinela `updated_by`; RPC `fn_claim_auvo_outbox_batch` para reivindicação atômica) — documentadas em `tasks.md` → Divergências | Claude (sessão Lucas) | ⏳ (AC implementados; gate real pendente de CI) |
| E01-S23 | Motor de sync Auvo (read path) — dispatcher de webhook genérico (User/Customer/Equipment/Ticket) + poller `pcm-auvo-pull` + auto-registro de webhook. Mesmo `design.md` de E01-S22 | — | Planejado — depende de E01-S22 | — | — |
| E01-S24 | Tipos de Tarefa (cron) — primeira entidade do catálogo, valida o motor ponta a ponta | — | Planejado — depende de E01-S23 | — | — |
| E01-S25 | Segmentos + Palavras-chave (cron) | — | Planejado | — | — |
| E01-S26 | Categorias (produto + equipamento) (cron) | — | Planejado | — | — |
| E01-S27 | Grupos de Clientes (cron) + Clientes CRUD (webhook Customer) | — | Planejado | — | — |
| E01-S28 | Funcionários (webhook User) — promove `pcm.tecnicos_cache` para tabela editável | — | Planejado — relaxa política deny-write de `0012` | — | — |
| E01-S29 | Equipamentos (webhook Equipment) — promove `pcm.equipamentos_cache` para tabela editável | — | **Bloqueado — contradiz decisão de E01-S16** ("Auvo é dono do equipamento, PCM não duplica"); precisa de ADR novo substituindo essa decisão antes de codar | — | ⛔ |
| E01-S30 | Ferramentas/Kits (produtos Auvo, cron) + alocação por técnico via `employee-product-stock` | — | Planejado | — | — |
| E01-S31 | Serviços (cron) | — | Planejado | — | — |
| E01-S32 | Equipes/Teams (cron) | — | Planejado | — | — |
| E01-S33 | Tickets (webhook Ticket) | — | Planejado | — | — |

...
| E00-S09 | Grupos e permissões por módulo — fundação: schema, resolver, hook JWT, Edge Function de gestão de usuário | [spec](../../specs/E00-S09-grupos-permissao-modulo/spec.md) · [design](../../specs/E00-S09-grupos-permissao-modulo/design.md) | Implementado, gates de código verdes (`lint:migrations`/Squawk) — `db-tests` (pgTAP) não rodou localmente (sem Docker), fica pro CI. Commits locais na branch `feat/E00-S09-grupos-permissao-modulo`, aguardando push/PR | Claude (sessão Lucas) | ⏳ |
| E00-S10 | Grupos e permissões por módulo — UI administrativa (grupos, usuários) e gating de sidebar | [spec](../../specs/E00-S10-grupos-permissao-ui/spec.md) | Implementado, `lint`/`typecheck`/`test` (75 verdes)/`build`/`arch:check` verdes — teste manual em browser pendente. Branch `feat/E00-S10-grupos-permissao-ui`, em cima de E00-S09, aguardando push/PR | Claude (sessão Lucas) | ⏳ |
...
| E01-S09 | Integração Auvo — Fundação: cliente HTTP, sync de clientes, criação de task ao entrar em `planejamento` | [spec](../../specs/E01-S09-integracao-auvo-fundacao/spec.md) · [design](../../specs/E01-S09-integracao-auvo-fundacao/design.md) | Implementado, gates de código verdes (`lint:migrations`/`audit-esteira`/`eval-spec-fidelity`) — Edge Functions (Deno) não puderam ser type-checked/executadas nesta sessão (sem Deno CLI), sem acesso ao mapeamento real da API Auvo pra confirmar formato exato de resposta. Branch `feat/E01-S09-integracao-auvo-fundacao` (baseada em `main`), aguardando push/PR | Claude (sessão Lucas) | ⏳ |

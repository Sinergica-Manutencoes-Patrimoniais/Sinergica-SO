...
| E01-S12 | Visão 360 do Cliente — página read-only por condomínio (v1: cadastro + backlog GUT + histórico de OS; +técnicos/equipamentos se E01-S11 fechar). PMOC/ativos/financeiro/WhatsApp adiados p/ fase 2 | [product](../../specs/E01-S12-visao-360-cliente/product.md) · [spec](../../specs/E01-S12-visao-360-cliente/spec.md) · [tasks](../../specs/E01-S12-visao-360-cliente/tasks.md) | **tasks.md pronto** (`@sm`, 2026-07-03 — 20 tasks, sem migration nova; confirmado que `E01-S11` ainda não existe nesta build, então AC-6 vai exercitar a degradação de verdade) — 3 OPEN-QUESTION não-bloqueantes: (1) como vincular técnicos a cliente (schema não suporta → v1 cobre só equipamentos); (2) separar cancelada de concluída no histórico (refinamento fase 2); (3) navegação até a Visão 360 — não existe roteamento por id nem lista de clientes hoje, wiring mínimo proposto como task não-bloqueante; próximo: `@dev` → implementar | Claude (sessão Lucas) | — |

...
export function ConversaPerfil({
  ...
  return (
    <div className="hidden h-full flex-col gap-4 overflow-y-auto rounded-lg border border-line bg-card p-4 xl:flex">
      <div>
        ...
        {conversa.contatoNome && (
          <p className="mt-1 text-caption text-ink-3">{conversa.contatoNome}</p>
        )}
        {!conversa.clientId && (
          <p className="mt-2 rounded-md border border-warning-soft bg-warning-soft px-2 py-1.5 text-caption text-warning">
        ...
        {podeVincular && (
          <div className="mt-3 space-y-2">
            <select
              ...
              className="input h-9 w-full text-caption"
            >
            ...
            <button
              ...
              className="btn-secondary inline-flex h-8 w-full items-center justify-center gap-1.5 text-caption disabled:opacity-50"
            >
      ...
      {conversa.handoffMotivo && (
        <div className="rounded-md border border-warning-soft bg-warning-soft px-2.5 py-2">
          <p className="text-caption font-semibold text-warning">Aguardando atendimento humano</p>
          <p className="mt-1 text-caption text-warning">{conversa.handoffMotivo}</p>
        </div>
      )}

      {conversa.ordemServicoId && (
        <div>
          <h4 className="text-caption font-semibold uppercase tracking-[0.12em] text-ink-3">
            Ordem de serviço
          </h4>
          <div className="mt-2 flex items-center gap-1.5 rounded-md border border-line px-2.5 py-1.5 text-caption text-ink-2">
